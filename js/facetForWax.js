...
vocode(carrierPattern, bands = [1000, 2000, 3000, 4000, 5000, 6000, 7000, 8000, 9000, 1000], q = 2.5) {
    if (typeof bands == 'number' || Array.isArray(bands) === true) {
      bands = new FacetPattern().from(bands);
    }
    let out_fp = new FacetPattern();
    for (let i = 0; i < bands.data.length - 1; i++) {
      out_fp.sup(this.bpf(bands.data[i], q).follow(1, 1)
        .times(carrierPattern.bpf(bands.data[i], q))
      )
    }
    this.data = out_fp.data;
    this.full().fadeoutSamples(SAMPLE_RATE / 30);
    return this;
  }
