...
const computeMagnitudes = (fftOutput) => {
    return fftOutput.map(x => Math.sqrt(x.real * x.real + x.imag * x.imag));
};
...
// attach to the global scope in the web worker's context (self)
self.Complex = Complex;
self.fft = fft;
self.ifft = ifft;
self.reverseBits = reverseBits;
self.computeMagnitudes = computeMagnitudes;
self.nextPowerOfTwo = nextPowerOfTwo;
