...
## sequencer
Advances to the next step in a 8-step sequence every time a signal connected to the `advance` inlet goes above 0.5. When a signal connected to the `reset` inlet goes above 0.5, the sequence will immediately move to the first step. The `output` signal produces the value for the current step, with slider ranges between 0 - 1. The `hit maximum` outlet produces a short 1 signal when the sequencer hits its maximum and wraps back around to its first value, and otherwise outputs a 0. The `step` outlet produces the number of the current step. Steps are zero-based, so the first step = step 0.
